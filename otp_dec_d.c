...
void decrypt(char msg[], char key[], int len)
{
	...
	int decNumber; // Var used to combine both numbers, and mod 27 to get new number	
	...
	for(i = 0; i < len; i++)
	{
		...
		decNumber = (messageNumber - keyNumber) + 27;
		decNumber = decNumber % 27;

		// Place new char in message
		msg[i] = getChar(decNumber); 
	}
	return;
}
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if(childId == 0)
		{
			...
			//printf("SERVER: Decrypted message length: %d\n", strlen(message)); fflush(stdout);
